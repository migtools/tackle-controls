...
curl -k -X POST "$1/controls/business-service" -H  "accept: application/json" -H  "Content-Type: application/json" -H "Authorization: Bearer "$access_token -d "{  \"name\": \"Credit Cards BS $STAKEHOLDER_ID_EMMET\",  \"description\": \"Internal credit card creation and management service\",  \"owner\": {    \"id\": $STAKEHOLDER_ID_EMMET  }}"|jq .;
